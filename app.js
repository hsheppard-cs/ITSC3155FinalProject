        ...
        success: function (data) { //on success calls this functions and passes the API response as the data parameter.
            ...
            $.each(data['data']['List'], function(i, item) {
                ...
                productListAdd = '<div class="col-sm-6 col-md-4 col-lg-3 mt-4" id="product'+item['id']+'">\n' +
                    ...
                    '                </div>\n' +  '<div class="card-footer"><button class="btn btn-info float-right btn-sm" onclick="addToWishlist('+item['id']+')">Wishlist</button></div>' +
                    '            </div>\n' +
                    '        </div>';
...
function addToWishlist($id) {
    ...
    let email =$.trim($('#email').val());
    email = email + "1"; //Key to get "Dummy Email" for wishlist
    data = {
        "product_id": $id,
        "email": email,
      }
...
function addToCart1(id5) {
    alert("Item added to your cart!");
    email1 = email.slice(0, email.length - 1); // Adding item to the actual email
    data = {
        "product_id": id5,
        "email": email1,
      }
...
function addToCheckout(id6) {
    
    email2 = email + "11"; // Adding key to checkout email
    data = {
        "product_id": id6,
        "email": email2,
      }
...
function toCheckout(){
    let email =$.trim($('#email').val()); //gets the user's email
    email11 = email + "1" + "1"; // Adding key to email
    //email validation

    if( email11 !='' ) {
        
        sessionStorage.setItem('email' + "1" + "1", email11); //setItem 'email11' in sessionStorage to be the user's email. You can access sessionStorage by sessionStorage.getItem().
        
        window.location.href = './Checkout.html'; //redirect to the shopping cart page
    } else {
        alert("Please enter your email at top of page."); //alert user since email is empty
    }
}
...
function toWishList(){
    let email =$.trim($('#email').val()); //gets the user's email
email1 = email + '1'; //Adding email to key
    //email validation

    if( email1 !='' ) {
        sessionStorage.setItem('email' + "1", email1); //setItem 'email1' in sessionStorage to be the user's email. You can access sessionStorage by sessionStorage.getItem().
